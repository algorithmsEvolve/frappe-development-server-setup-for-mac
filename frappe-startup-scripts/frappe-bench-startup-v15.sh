...
source ~/.nvm/nvm.sh && nvm deactivate && nvm install 18 && nvm use 18 && nvm alias default 18 && nvm alias default node
...
# initialize bench
bench init --skip-redis-config-generation --python "/home/frappe/.pyenv/shims/python" --frappe-branch version-15 frappe-bench --verbose
...
# create a new site
bench new-site $SITE_NAME --mariadb-root-password root --admin-password administrator --no-mariadb-socket --db-name erpnext --verbose
